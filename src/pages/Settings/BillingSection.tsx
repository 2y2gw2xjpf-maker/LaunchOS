...
export default function BillingSection() {
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            {isPaid && currentTier ? (
              ...
              <p className="text-slate-400">Kostenloser Plan mit eingeschränkten Features</p>
            )}
          </div>
      ...
      {/* Usage Stats */}
      <div>
        <h4 className="text-lg font-semibold text-white mb-4">Nutzung</h4>
      ...
      {isPaid && (
        <div>
          <h4 className="text-lg font-semibold text-white mb-4">Zahlungshistorie</h4>
          <div className="bg-slate-700/30 rounded-lg overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-600">
                  <th className="px-4 py-3 text-left text-sm font-medium text-slate-400">Datum</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-slate-400">Beschreibung</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-slate-400">Betrag</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-slate-400">Status</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-slate-700">
                  <td className="px-4 py-3 text-sm text-slate-300">15. Jan 2026</td>
                  <td className="px-4 py-3 text-sm text-slate-300">{currentTier?.name} Plan - Monatlich</td>
                  <td className="px-4 py-3 text-sm text-white text-right">{formatPrice(currentTier?.price ?? 0)}</td>
          ...
          <p className="text-sm text-slate-500 mt-2">
            ...
            <button onClick={handleManageBilling} className="text-purple-400 hover:text-purple-300">
...
function UsageCard({ label, used, limit }: UsageCardProps) {
  ...
  return (
    <div className="bg-slate-700/30 rounded-lg p-4">
      ...
      {isAtLimit && (
        <p className="text-xs text-red-400 mt-1">Limit erreicht - Upgrade erforderlich</p>
      )}
    </div>
  );
}
