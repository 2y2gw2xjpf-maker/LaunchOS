        ...
        <PageContainer withSidebar maxWidth="wide">
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-charcoal/60">Checklist wird geladen...</p>
            </div>
          </div>
        </PageContainer>
      </div>
      ...
      <div className="min-h-screen bg-cream">
        ...
        <PageContainer withSidebar maxWidth="wide">
          <div className="flex flex-col items-center justify-center py-20">
            <h2 className="text-xl font-semibold text-charcoal mb-2">Checklist nicht gefunden</h2>
      ...
      <PageContainer withSidebar maxWidth="wide">
        ...
        >
          <nav className="flex items-center gap-2 text-sm mb-4">
            ...
            <Link to="/toolkit/checklists" className="text-charcoal/50 hover:text-purple-600">
              Checklisten
            </Link>
            <ChevronRight className="w-4 h-4 text-charcoal/40" />
            <span className="text-charcoal font-medium truncate">{checklist.title}</span>
          </nav>
        ...
        <motion.div
          ...
          className="bg-white/80 backdrop-blur-sm rounded-2xl border border-purple-100 p-6 mb-8"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-display text-display-sm text-charcoal">{checklist.title}</h1>
          ...
          {checklist.description && (
            <p className="text-charcoal/60 mb-6">{checklist.description}</p>
          )}
          ...
          {progress && (
            <div className="bg-cream rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-charcoal/70">Fortschritt</span>
                <span className="text-sm font-bold text-charcoal">{progress.percentage}%</span>
              </div>
            ...
            >
              ...
              <div className="space-y-3">
                {items.map((item) => (
                  <div
                    key={item.id}
                    className={`bg-white/80 backdrop-blur-sm rounded-xl border transition-all ${
                      item.isCompleted
                      ...
                      <div className="flex items-start gap-3">
                        <button
                          ...
                          className={`mt-0.5 transition-colors ${
                            item.isCompleted ? 'text-green-600' : 'text-charcoal/40 hover:text-purple-600'
                          }`}
                        >
                          {item.isCompleted ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h3 className={`font-medium ${
                              item.isCompleted ? 'text-charcoal/50 line-through' : 'text-charcoal'
                            }`}>
                              {item.title}
                            </h3>
                            {item.isCritical && (
                          ...
                          {item.description && (
                            <p className="text-sm text-charcoal/50 mt-1">{item.description}</p>
                          )}
                        </div>
                      ...
                      {expandedItem === item.id && (item.helpText || item.helpLink) && (
                        ...
                        >
                          <div className="px-4 pb-4 pt-0 border-t border-gray-100 ml-9">
                            {item.helpText && (
                              <p className="text-sm text-charcoal/60 mt-3 bg-purple-50 p-3 rounded-lg">
                                {item.helpText}
                              </p>
                            )}
                            {item.helpLink && (
